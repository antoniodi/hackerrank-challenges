...
class AreaUnderCurve {
  ...
  def f(coefficients: List[Int], powers: List[Int], x: Double): Double =
  {
    ...
    coefficients.zip(powers).map {
      case (coefficient, power) => coefficient * Math.pow(x, power)
    }.sum
  }
  ...
  {
    val step = 0.001
    def evalFunction(start: Double, end: Double, volume: Double = 0): Double = {
      if (start <= end)
        evalFunction(start + step, end, volume + func(coefficients, powers, start) * step)
      else volume
    }
    evalFunction(lowerLimit, upperLimit)
  }
